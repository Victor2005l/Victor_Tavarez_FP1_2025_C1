...
int Busca(int A[], int T, int E)  /* Esta funcion se utiliza para localizar el elemento E en el arreglo unidimensional A.
Si se encuentra, la funcion regresa la posicion correspondiente. En caso contrario regresa 0. */
{
    int RES, I = 0, BAN = 0;
    while ((I < T) && (E >= A[I]) && !BAN) /* Observa que se incorpora una nueva condici�n. */
        if (A[I] == E)
            BAN++;
        else
            I++;
