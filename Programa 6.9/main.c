...
funci�n del m�dulo (residuo) a un arreglo unidimensional. */
...
void main(void)
{
    ...
    while (TAM > 10 || TAM < 1); /* Se verifica que el tama�o sea v�lido */
...
/* La funci�n Lectura se utiliza para leer un arreglo bidimensional cuadrado
de tipo entero. */
{
    int I, J;
    for (I = 0; I < N; I++)
...
/* Esta funci�n se utiliza tanto para calcular el m�dulo entre el �ndice del
...
/* Esta funci�n calcula el cociente entre una productoria y una sumatoria. */
{
    int I;
...
/* Esta funci�n obtiene el resultado de una sumatoria. */
{
    int I;
    float SUM = 0.0;
    for (I = 0; I < N; I++)
    {
        SUM += A[N][I]; /* Nota: Aqu� probablemente quieras sumar a A[I][N] */
    }
    return (SUM);
}

float Mod2(int A[][10], int N, int M)
/* Esta funci�n obtiene el resultado de la productoria. */
{
    int I;
    float PRO = 1.0;
    for (I = N; I < M; I++)
...
/* Esta funci�n se utiliza para escribir un arreglo unidimensional de tipo
real de N elementos. */
{
    int I;
    for (I = 0; I < N; I++)
