...
const int PRO = 24; // N�mero de provincias
const int MES = 12; // N�mero de meses
const int A�O = 10; // N�mero de a�os
...
{
    int I, J, EMEN = -1; // Inicializaci�n de EMEN
    float ELLU = 1000, SUM;
    for (I = 0; I < F; I++)
    {
        SUM = 0;
...
la provincia 18 en el quinto a�o. Escribe adem�s el registro correspondiente. */
{
    ...
    for (J = 0; J < C; J++)
    {
        if (A[F - 1][J][P - 1] > ELLU) // Asegura que F-1 accede a la provincia 18
        {
            ELLU = A[F - 1][J][P - 1];
            EMES = J;
        }
